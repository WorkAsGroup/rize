...
import { setLoading, setError, setSuccess, setExamQuestions, setActiveSubjectId, setExamPatternSections, setQuestionDetails, setQuestionLoading, setUniqueeId, setExamSessionId, setFinishTest, setAutoSaveId, setActiveQuestionIndex, setExamDuration, setReloadQuestion, 
  setAutoSaveTime, setFinishedData } from '../slices/examSlice';
import { 
  getPreExam,
  getPatternSelection,
  getPreExamdata,
  ...
  getAutoSaveData,
  getAutoSaveTime,
  addAutoSaveData,reloadQuestion, 
  addAnalytics} from "../../core/CommonService";
...
console.log(response, "responseData", params) 
        if (response.statusCode === 200) {
            if (data.length > 0) {
                ...
                // console.log("qhbuqrhfbqwrhbqwrhbqhpwr", data)
...
export const patternSectionsThunk = (params, type) => async (dispatch, getState) => {
    try {
        dispatch(setLoading(true));
        ...
        const uniqueSubjectsFromExamQuestions = examQuestions.reduce((unique, item) => {
            if (item.subject !== null && item.subject !== undefined && !unique.includes(item.subject)) {
...
console.log(params, "paranfkjenf")
    ...
    try {
        ...
        if (data.statusCode === 200) {
            ...
            await dispatch(questionsThunk({
                "exam_paper_id": params.exam_paper_id,
                "exam_session_id": params?.exam_type === "previous" ? examSessionId : params.exam_session_id,
               "type": params?.type 
...
export const fetchAutoSaveDataThunk = (params) => async (dispatch) => {
    try {

        const response = await getAutoSaveData(params);
        const data = response.data[0];
console.log(data,params,data.examtimer, "iejfoiwejfoiwenfowei")
        const activeSubjectId = JSON.parse(data.questions_data).find(q => q.slno === data.qsno)?.subject_id;
    ...
    try {
        ...
        if (response.data.statusCode === 200) {
            if (data.length > 0) {
                ...
                const isExistingQuestion = data.find((item) => item.id === params.question_id);

                if (isExistingQuestion) {
                    filteredExamQuestions = filteredExamQuestions.map((question) => question.id === isExistingQuestion.id ? {
                        ...question,
                        ...isExistingQuestion,
                        selection_type: 2,
                    }
                    ...
                    if (!filteredExamQuestions.find((q) => q.id === isExistingQuestion.id)) {
