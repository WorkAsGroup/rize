...
const LeaderBoard = ({ studentExamId }) => {
  ...
  useEffect(() => {
    if (champ && champ.length > 0) {
      const filteredData = champ.filter(
        (item) => item.report_level === leaderBoardValue
      );
  ...
  const renderItem = ({ item, index }) => {
    return (
      ...
      >
        ...
        >
          <Text style={{ color: theme.white }}>{item.name[0]}</Text>
        </View>
        <View style={{ height: 40, width: 50 }}></View>
        <View
          style={{
            flexDirection: "row",
        ...
        >
          ...
          >
            {item.count === 0 ? (
              <Text style={{ fontSize: 21, color: "#2575FC", marginRight: 5 }}>
                =
              </Text>
            ) : item.is_increase ? (
              <>
      ...
      >
        <View>
          ...
          <Text style={[styles.subText, { marginBottom: 10 }]}>
            Checkout your leaderboard score
          </Text>
        </View>
