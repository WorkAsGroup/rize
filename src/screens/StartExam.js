...
const sanitizeHtml = (text) => {
    ...
    text = text.replace(/<img /g, "<img style='display:inline-block; vertical-align:middle;' "); // Force images inline
...
//             if (selectedOption) {
//               // You can add any logic here based on option change
//             }
...
//   }, []); 
  ...
  useEffect(() => {
    const questionInitialTime = exams.filter((item) => item.question_id === selectedNumber?.id)[0]?.question_time || 0
    setTimeElapsed(questionInitialTime);
}, [selectedNumber]);
...
useEffect(() => {
    const timerInterval = setInterval(() => {
        setTimeElapsed((prevTime) => {
            ...
            if (newTime !== previousTimeRef.current) {
                if (selectedNumber) {
                    const updatedQuestionDetails = exams.map((item) =>
                        item.slno === exams[selectedNumber] + 1
                            ? {
                                ...item,
  ...
  const tagsStyles = {
    img: {
      display: "inline-flex", // ✅ Keeps images inline with text
      maxWidth: "100%",
      height: "auto",
      verticalAlign: "middle", // ✅ Aligns image properly with text
    },
    p: {
      display: "inline", // ✅ Ensures paragraphs stay inline
    },
    div: {
      display: "inline-flex", // ✅ Ensures divs don’t force a new line
    },
    span: {
      display: "inline-flex", // ✅ Wraps images inside inline elements
    },
  };
  ...
  useEffect(() => {
    if (exams.length > 0) {
      ...
      console.log(initialTime, route.params.obj.exam_duration, "inital");
  ...
  useEffect(() => {
    const loadQuestionStartTime = async () => {
      try {
        const storedStartTime = await AsyncStorage.getItem(`questionStartTime_${selectedNumber}`);
        if (storedStartTime) {
          setQuestionStartTime(parseInt(storedStartTime, 10));
          const initialElapsed = Math.floor((Date.now() - parseInt(storedStartTime, 10)) / 1000)

          const questionInitialTime = exams.find((item) => item.question_id === selectedNumber?.id)?.question_time || 0;
          setTimeElapsed(initialElapsed + questionInitialTime);
        } else {
          const newStartTime = Date.now();
          setQuestionStartTime(newStartTime);
          await AsyncStorage.setItem(`questionStartTime_${selectedNumber}`, newStartTime.toString());
          const questionInitialTime = exams.find((item) => item.question_id === selectedNumber?.id)?.question_time || 0;
          setTimeElapsed(questionInitialTime); 
        }

      } catch (error) {
        console.error("Error loading/saving question start time:", error);
      }
    };


    if (selectedNumber && exams.length > 0 ) { 
      loadQuestionStartTime();
    }
  ...
  const questionHtml =
    exams.length > 0 ? exams[selectedNumber - 1]?.question : "";
  const questionHtml1 =
    exams.length > 0 ? exams[selectedNumber - 1]?.compquestion : "";
  ...
  }, [filteredQuestionNumbers, selectedNumber]);
                    ...
                    >
                      {allNum.map((num) => {
                        ...
                        return (
                          <View
                            key={num}
                            ref={(el) => (numberCircleRefs.current[num] = el)}
                            onLayout={({
                              nativeEvent: {
                                layout: { width },
                              },
